      ...
      <div>
        ...
        {day !== null &&
          ...
          (() => {
            ...
            const countedRoutines = day.routines.filter(
              (r) => r.status !== "skipped",
            );
            const isEmpty =
              day.routines.length === 0 && day.randomTasks.length === 0;
                ...
                <section className="flex flex-col gap-2">
                  <SectionHeader
                    title="Routines"
                      ...
                      countedRoutines.filter((r) => r.status === "completed")
                        .length
                    }
                    total={countedRoutines.length}
                  />
                  {day.routines.length === 0 ? (
                    <p className="text-[12px] italic text-[var(--text-tertiary)] pt-1">
                      None scheduled
                    </p>
                  ) : (
                    day.routines.map((r) => (
                ...
                <section className="flex flex-col gap-2">
                  <SectionHeader
                    title="Tasks"
                      ...
                      day.randomTasks.filter((t) => t.status === "completed")
                        .length
                    }
